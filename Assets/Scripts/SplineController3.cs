...
public enum eOrientationMode3 { NODE = 0, TANGENT }

[AddComponentMenu("Splines/Spline Controller")]
[RequireComponent(typeof(SplineInterpolator))]
public class SplineController3 : MonoBehaviour
{
	public GameObject SplineRoot;
	public float Duration = 10;
	public eOrientationMode3 OrientationMode = eOrientationMode3.NODE;
	...
	void SetupSplineInterpolator(SplineInterpolator interp, Transform[] trans)
	{
		...
		{
			if (OrientationMode == eOrientationMode3.NODE)
			{
				interp.AddPoint(trans[c].position, trans[c].rotation, step * c, new Vector2(0, 1));
			}
			else if (OrientationMode == eOrientationMode3.TANGENT)
			{
				Quaternion rot;
				if (c != trans.Length - 1)
